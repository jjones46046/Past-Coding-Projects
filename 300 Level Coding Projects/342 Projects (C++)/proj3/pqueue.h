...
private:
  ...
  void rebuild(Node* node, PQueue* copy);

  void preOrderCopy(Node* rhsroot, Node* node);

  Node* preOrderOp(Node* rhsroot, Node* node);

  Node* mergeRecursive(Node* rhsroot, Node* root);

  void dumpInOrder(Node* root) const;

  void clearRecursive(Node* heap);
